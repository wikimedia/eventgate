...
function makeExtractStreamName(options) {
    // TODO: if no stream_field in event, fallback to sanitized schema uri
...
function makeExtractKey(options = {}) {
    if (_.isUndefined(options.key_field)) {
        ...
        return (event, context = {}) => {
            return objectGet(event, options.key_field);
        };
    }
}

/**
 * Creates a function that extracts a kafka partition, uses options.partition_field.
 ...
 *      dotted object path.
 * @return {function(Object, Object): integer}
 */
function makeExtractPartition(options = {}) {
    if (_.isUndefined(options.partition_field)) {
        ...
        return (event, context) => {
            return _.get(event, options.partition_field);
        };
    }
}
 ...
 */
function makeProduce(options, producer, hastyProducer) {
    ...
    return (event, context = {}) => {
        ...
        if (hastyProducer && _.get(context, 'req.query.hasty', false)) {
            return hastyProducer.produce(topic, partition, serializedEvent, key);
        } else {
            return producer.produce(topic, partition, serializedEvent, key);
        }
    };
}
