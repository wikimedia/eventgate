...
function makeExtractSchemaUri(options) {
    ...
    return (event, context = {}) => {
        try {
            const schemaUri = objectFindAndGet(event, schemaUriField);
            return schemaUri;
        } catch (err) {
            ...
            if (err instanceof PropertyNotFoundError) {
                throw new EventSchemaUriMissingError(
                    ...
                    `Event must have a field named one of '${schemaUriField}'`
                );
            } else {
                throw err;
            }
...
/**
 * Returns a function that searches schema_base_uris/uri schema content, and returns
 * the first one found.
 * @param {Object} options
 * @param {string} options.schema_uri_field
 *      Used to extract the event's schema URI.
 * @param {string} options.schema_base_uris
 *      If set, these are prefixed to un-anchored schema URIs.
 ...
 */
function makeGetSchema(options) {
    return (uri) => {
...
function makeValidate(options, logger) {
    ...
    const eventValidator = new EventValidator({
        ...
        getSchema: makeGetSchema(options),
        log: logger
    });
