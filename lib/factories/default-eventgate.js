...
async function eventGateFactory(options, logger, metrics) {
    ...
    const kafkaProducer = await kafka.producerFactory(
        'guaranteed',
        options.kafka.conf,
    ...
    if (options.kafka.conf['statistics.interval.ms'] && metrics) {
        let rdkafkaStatsd;
        try {
            rdkafkaStatsd  = require('node-rdkafka-statsd');
        } catch (err) {
            logger.fatal(
                err,
                'node-rdkafka-statsd could not be required. Make sure it is installed properly.'
            );
            process.exit(1);
        }
