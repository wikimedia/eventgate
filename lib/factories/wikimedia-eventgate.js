...
async function makeKafkaProducer(options, logger, metrics, hasty = false) {
    ...
    // Use the service name and producerType in the guaranteed Kafka client.id
    const producerKafkaConf = _.cloneDeep(options.kafka.conf);
    _.defaults(producerKafkaConf, { 'client.id': `${options.name}-producer-${producerType}` });

    const producer = await kafka.GuaranteedProducer.factory(
        producerKafkaConf,
    ...
    // innocuous start up connection failures.

    if (logger) {
        // Log rdkafka errors with our logger
        producer.on('event.error', (error) => {
            // broker transport failures are noisy and misleading, they haappen whenever
            // a Kafka broker connection reaper terminates a client connection.
            if (error.message !== 'broker transport failure') {
                logger.error({ error }, `${producerType} producer encountered rdkafka error event: ${error.message}`);
            }
        });

        // If kafka.conf.event_cb: true, then we will enable rdkafka logging.
        // logAtSyslogSeverity will log at the appropriate bunyan severity level.
        ...
        if (options.kafka.conf.event_cb) {
            producer.on('event.log', (rdkafkaLog) => {
                logAtSyslogSeverity(
                    logger,
                    rdkafkaLog.severity,
                    { rdkafka_facility: rdkafkaLog.fac },
                    rdkafkaLog.message
                );
            });
        }
