...
function makeMapToErrorEvent(options) {
    return (error, event, context = {}) => {
        ...
        const streamNameField = _.isArray(options.stream_field) ?
            options.stream_field[0] :
            options.stream_field;
        /* eslint-enable */

        _.set(eventError, schemaUriField, options.error_schema_uri);
        _.set(eventError, streamNameField, options.error_stream);
...
async function makeWikimediaValidate(options, logger) {
    ...
    // On SIGHUP, reload EventValidator's Ajv instance and the streamConfig.
    process.on('SIGHUP', async() => {
        logger.info('Received SIGHUP, clearing cached schemas and reloading stream config.');
        await init();
        logger.debug('Finished reloading stream config and clearing cached schemas after SIGHUP.');
    });
    ...
    async function ensureEventAllowedInStream(event, streamName) {
        ...
        if (_.isUndefined(schemaTitle)) {
            throw new UnauthorizedSchemaForStreamError(
                `Schema at ${schemaUri} must define a 'title' field.`
            );
        }

        // Look up the stream config for this streamName.
        // This will return the first matching stream config.
        const streamConfigKey = streamConfigKeys.find(key => stringMatches(streamName, key));
        if (_.isUndefined(streamConfigKey)) {
            throw new UnauthorizedSchemaForStreamError(
                `Event of schema at ${schemaUri} is not allowed in stream ${streamName}; ` +
        ...
        if (_.isUndefined(allowedSchemaTitle)) {
            throw new UnauthorizedSchemaForStreamError(
                `Event of schema at ${schemaUri} is not allowed in stream ${streamName}; ` +
                `${streamName} does not have a configured ${streamConfigSchemaTitleField}.`
            );
        }

        if (schemaTitle !== allowedSchemaTitle) {
            throw new UnauthorizedSchemaForStreamError(
                `Schema with title ${schemaTitle} is not allowed in stream ${streamName}; ` +
