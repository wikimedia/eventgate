...
async function wikimediaEventGateFactory(options, logger, metrics) {
    ...
    if (options.kafka.conf['statistics.interval.ms'] && metrics) {
        ...
        guaranteedProducer.on(
            'event.stats',
            // eventgate.brokers.mws_mediawiki-vagrant_dev_9092_0.txbytes
            // eventgate.rdkafka.producer.guaranteed.eventgate.topics.datacenter1_test_event.partitions.0.txbytes
