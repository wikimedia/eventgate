 ...
 *
 * The Wikimedia validate function created will reactr to SIGHUPs by reloading
 ...
 * error events for EventInvalidErrors.
...
async function makeWikimediaValidate(options, logger) {
    ...
    const extractStream = makeExtractStream(options);
    ...
    if (options.schema_base_uri) {
        logger.info(`Will look for relative schema_uris in ${options.schema_base_uri}`);
    }
    ...
    async function ensureEventAllowedInStream(event, stream) {
        ...
        if (_.isUndefined(streamConfigKey)) {
            throw new UnauthorizedSchemaForStreamError(
                `Event of schema at ${schemaUri} is not allowed in stream ${stream}; ` +
        ...
        if (schemaTitle !== allowedSchemaTitle) {
            throw new UnauthorizedSchemaForStreamError(
                `Schema with title ${schemaTitle} is not allowed in stream ${stream}; ` +
...
function makeExtractTopic(options) {
    ...
    return (event, context = {}) => {
        const stream = extractStream(event);
        return options.topic_prefix ?  options.topic_prefix + stream : stream;
    };
}
 ...
 *      If given, this will be prefixed to the value extracted from stream_field
 *      and used as the topic in Kafka.
 * TODO: implement these or remove doc.
 ...
 * will be created and produced upon EventInvalidErrors.
 * @param {Object} options
 * @param {string} options.schema_uri_field
 *      Used to extract the event's schema URI.
 * @param {string} options.schema_base_uri
