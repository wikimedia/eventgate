...
async function wikimediaEventGateFactory(options, logger, metrics) {
    ...
    // Use the service name in the guaranteed Kafka client.id
    const guaranteedProducerKafkaConf = _.cloneDeep(options.kafka);
    _.defaults(guaranteedProducerKafkaConf, { conf: { 'client.id': `${options.name}-producer-guaranteed` } });
    options.kafka.conf['client.id'] = 'eventgate-guarunteed';
    const guaranteedProducer = await kafka.GuaranteedProducer.factory(
        guaranteedProducerKafkaConf.conf,
        guaranteedProducerKafkaConf.topic_conf
    ).delay(100);
    // ^ rdkafka takes a small amount of time to finish connecting to brokers.
    // Pause before registering the error logger so we don't pollute logs with
    // innocuous 'Broker transport failure' errors during connect.
    // Log rdkafka errors with our logger
    guaranteedProducer.on('event.error', (error) => {
        logger.error({ error }, `GuaranteedProducer encountered rdkafka error event: ${error.message}`);
    });

    // Use the service name in the hasty Kafka client.id
    const hastyProducerKafkaConf = _.cloneDeep(options.kafka);
    _.defaults(hastyProducerKafkaConf, { conf: { 'client.id': `${options.name}-producer-hasty` } });
    options.kafka.conf['client.id'] = 'eventgate-hasty';
    const hastyProducer = await kafka.HastyProducer.factory(
        hastyProducerKafkaConf.conf,
        hastyProducerKafkaConf.topic_conf
    ).delay(100);
    // Log rdkafka errors with our logger
    guaranteedProducer.on('event.error', (error) => {
        logger.error({ error }, `HastyProducer encountered rdkafka error event: ${error.message}`);
    });

    // If we Kafka is set with statistics reporting turned on,
    ...
    if (options.kafka.conf['statistics.interval.ms'] && metrics) {
        logger.info('Enabling Kafka metrics reporting for both guaranteed and hasty Kafka producers');
        guaranteedProducer.on(
            'event.stats',
            rdkafkaStatsd(metrics.makeChild('rdkafka.producer.guaranteed'))
        );
        hastyProducer.on(
            'event.stats',
            rdkafkaStatsd(metrics.makeChild('rdkafka.producer.hasty'))
        );
    }
