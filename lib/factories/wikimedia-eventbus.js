...
/**
 * Returns a new mapToErrorEvent function that uses conf.error_schema_uri, and conf.error_stream
 ...
 */
function mapToEventErrorFromConf(conf) {
    return (event, error) => {
        const eventError = {
...
// TODO
function ensureSchemaAllowedInStreamFromConf(conf) {
...
// TODO
async function wikimediaValidateFromConf(conf, logger) {
    ...
    const ensureSchemaAllowedInStream = await ensureSchemaAllowedInStreamFromConf(conf);
    return (event) => {
...
function wikimediaEventbusFromConf(conf, logger) {
    conf = setConfDefaults(conf);

    const validatePromise = wikimediaValidateFromConf(conf, logger);
    ...
    .spread((validate, produce) => {
        return new Eventbus({
            ...
            log: logger,
            mapToEventError: mapToEventErrorFromConf(conf)
        });
    });
}
...
module.exports = {
    factory: wikimediaEventbusFromConf,
    wikimediaValidateFromConf,
    ensureSchemaAllowedInStreamFromConf,
    UnauthorizedSchemaForStreamError,
    mapToEventErrorFromConf
};
