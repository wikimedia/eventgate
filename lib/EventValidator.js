 ...
 * $id field.
 *
 * This class's validate method is intended to be used as an Eventbus validate function.
 */
class EventValidator {
    ...
    loadSchema(uri) {
        ...
        .then((schema) => {
            ...
            if (
                _.has(schema, '$id') && schema.$id !== uri &&
                _.isUndefined(this.ajv.getSchema(uri))
            ) {
                this.log.debug(
                    `Additionally caching schema with $id '${schema.$id}' ` +
                    `by schema URI '${uri}'`
                );
