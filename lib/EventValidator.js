...
class EventValidator {
    ...
    loadSchema(uri) {
        ...
        .then((schema) => {
            ...
            // Assume that all schemas hosted at metaSchemaUriRegex (json-schema.org by default)
            // are meta schemas. This allows us to support automatically adding
            // JSONSchema meta schemas (schemas for schemas) and avoid infinite loadSchema
            // recursion. Since meta schemas usually reference themselves with $schema,
            ...
            if (this.metaSchemaUriRegex && url.match(this.metaSchemaUriRegex)) {
                this.log.info(`Adding schema at ${url} as meta schema.`);
