...
const eventValidatorDefaultOptions = {
 ...
 *
 * This class's validate method is intende to be used as the Eventbus validate function.
 */
class EventValidator {
    /**
     * @param {Object} options
     * @param {function<(Object) => string} options.extractSchemaUri given an event, this returns
     *      its JSONSchema URI. Default is to return the $schema field of an event.
     * @param {function<(Object) => string} options.resolveSchemaUri given a uri, this will
     *      return a resolved schema url. Default is to just return the uri as given.
     * @param {Object} options.ajvConfig Any extra options to pass to new Ajv().
     * @param {Object} options.log an instantiated bunyan logger instance.
     */
    constructor(options = {}) {
        _.defaults(this, options, eventValidatorDefaultOptions);
    ...
    validatorFor(event) {
        ...
        if (validator) {
            ...
            .catch((error) => {
                throw new EventSchemaLoadError(eventSchemaUri, error);
            })
            .then((schema) => {
