...
// TODO this error should be generic!  Should not be implementation
...
class Eventbus {
     ...
     */
    constructor(options = {}) {
        _.defaults(this, options, eventbusDefaultOptions);

        if (_.isUndefined(this.validate)) {
            throw new Error('Cannot instatiate Eventbus, must provide a validate function');
        }
        if (_.isUndefined(this.produce)) {
            throw new Error('Cannot instatiate Eventbus, must provide a produce function');
        }
    }
    ...
    async _processEvent(event) {
        ...
        try {
            ...
            const produceResult = await this.produce(event);
    ...
    process(events) {
        ...
        .then((results) => {
            ...
            // TODO: maybe we should only produce invalid events?
            // Or maybe we need another level of error?  We don't
            // want to produce all incoming bad data to an error topic no matter what.
