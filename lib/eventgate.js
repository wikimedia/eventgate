 ...
 * will be processed using this EvengGate instance.
 * If mapToErrorEvent returns null for a given error, no error event will be
 * produced for the error & event.
 *
 * Errors encoutered during error event processing will not be handled
    ...
    async _processEvent(event, context = {}) {
        ...
        try {
            ...
            if (err instanceof EventInvalidError) {
                this.log.warn(`${this.eventRepr(event)} failed schema validation.`);
