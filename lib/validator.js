...
const defaultAjvConfig = { useDefaults: true, schemaId: 'auto', allErrors: true };
/**
...
class ValidatorCache {
    ...
    constructor(
        ...
        this.log = loggerFactory(logger, { name: 'ValidatorCache' });
    }
    ...
    validatorFor(url) {
        if (!this.validatorCache.has(url)) {
            this.log.info(`Getting new validator for schema at ${url}`);
            return this.getValidator(url).then((v) => {
                this.validatorCache.set(url, v);
                return v;
            });
        } else {
            this.log.trace(`Using cached validator for schema at ${url}`);
