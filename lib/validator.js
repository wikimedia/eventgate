...
class EventValidator {
     ...
     * also used by the validatorFor function to load a schema
     * from an event's configured schema_uri_field (usually $schema).
     *
     ...
     * through this.resolveSchemaUri before attempting to download that content.
     * This returns a Promise of the schema content at the uri.
     *
     ...
     * to the enclosing schema's $id field.
     ...
     * As such, we need to make sure that all event schema URIs, including
    ...
    validatorFor(event) {
        // The likely relative schemaUri for this event.
        // If it is absolute, it will not be modified.
        const eventSchemaUri = this.extractSchemaUri(event);
        // The absoulte schema for this event.
        // const schemaUrl = this.resolveSchemaUri(schemaUri);
        ...
        if (validator) {
            ...
            .then((schema) => {
                ...
                // .then((validator) => {
                //     //  TODO validator.schema is not derefernced!
                //     // That's ok for validation, but if we wanted
                //     // to we couldn't use AJV to render dereferenced schema
                //     // versions (in a git commit hook).
                //     this.log.debug(
                //         { schema: validator.schema },
                //         `Compiled event schema at ${eventSchemaUri}`
                //     );
                //     return validator;
                // });
            });
    ...
    validate(event) {
        ...
        // can we .catch Ajv.ValidationError?
